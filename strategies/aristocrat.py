...
BASE_URL = "https://aristocrat.wd3.myworkdayjobs.com"

class AristocratStrategy(ScrapingStrategy):
    def fetch(self, url):
        self.driver.get(url)
            ...
            try:
                ...
                if meta_div:
                    for dl in meta_div.find_all('dl'):
                        ...
                        value = dl.find('dd')
                        if label and value:
                            if 'location' not in label.get_text(strip=True).lower():
                ...
                print(f"Error parsing Aristocrat card: {e}")
                continue
